...
extends CommandBase
{
	...
	public SetmoneyCommand() {
		super(2, "/setmoney <Spieler> <Betrag>");
	}
	...
	public boolean run(CommandSender sender, Player p, String[] args) {
		...
		try {
			...
			String reason;
			if(p != null) {
				reason = "Gesetzt von " + p.getName();
			} else {
				reason = "Gesetzt per Konsole";
			}
			MoneyAPI.setMoney(UUID.fromString(uuid), betrag, reason);
			sender.sendMessage("§aDer Kontostand von §6" + args[0] + " §awurde auf §6" + DecimalFormat.getNumberInstance(Locale.GERMAN).format(betrag) + "$ §agesetzt.");
			return true;
